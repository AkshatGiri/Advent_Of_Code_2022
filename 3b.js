...
const elf_groups = new Array(compartments.length / 3)
  .fill(0)
  .map((_, index) => [
    compartments[index * 3],
    compartments[index * 3 + 1],
    compartments[index * 3 + 2],
  ])

let common_items_priority_sum = 0

elf_groups.forEach(([comp1, comp2, comp3]) => {
  const common_items = new Set(
    ...comp1
      .split('')
      .filter((item) => comp2.includes(item) && comp3.includes(item)),
  )

  common_items.forEach((item) => {
    common_items_priority_sum += get_item_priority(item)
  })
})

function get_item_priority(item) {
...
console.log(common_items_priority_sum)
