...
const monkeys = input.split('\n\n').map((monkey_str) => {
  ...
  const starting_items = items.split(': ')[1].split(',').map((str) => parseInt(str))

  const operation_fn = (num) => {
    if(operation.includes('*')) {
      const [x, y] = operation.split(' = ')[1].split(' * ')
      const a = x === 'old' ? num : parseInt(x)
      const b = y === 'old' ? num : parseInt(y)
      return Math.floor((a * b) / 3)
    }
...
// const monkeys_inpsections = monkeys.map(() => 0)
